...
void SchedCross(DTVehicle *pCar, int clk) {
    ...
    //The following four if statements create output files for next simulation
    if (pCar->from == East ) {
        fprintf(d0, "%d,%.2f,%d,%d,%.2f,%d\n", pCar->carID, pCar->etd, pCar->from, pCar->speed, pCar->dist, pCar->type);
    }
    if (pCar->from == South ) {
        fprintf(d1, "%d,%.2f,%d,%d,%.2f,%d\n", pCar->carID, pCar->etd, pCar->from, pCar->speed, pCar->dist, pCar->type);
    }

    //Note that there is a 2 in the print statement to ensure that when this file gets feed to the next intersection,
    //all vehicles are marked as arriving from direction 2 which equates to WEST.
    if ((pCar->from == South & pCar->to == Right) || (pCar->from == West & pCar->to == Straight) || (pCar->from == North & pCar->to == Left)) {
        fprintf(d2, "%d,%.2f,%d,%d,%.2f,%d\n", pCar->carID, pCar->etd, 2 , pCar->speed, pCar->dist, pCar->type);
    }

    if (pCar->from == North ) {
        fprintf(d3, "%d,%.2f,%d,%d,%.2f,%d\n", pCar->carID, pCar->etd, pCar->from, pCar->speed, pCar->dist, pCar->type);
    }
...
int main(int argc, char *argv[]) {
    ...
    //****************************************************
    if (!readParameters("/Users/alejflor/thesis/02FifoLazy/02-fifolazy-param.dat"))
    {
        fprintf(stderr, "Go to the main section of 02-fifolazy.c and set the parameter path file!\n");
        return 1;
    }
    ...
    //These directories determin where the output of this simulations
    //should be stored to be used on the multiple intersection simulation


    d0 = fopen(strcat(outputpath[7],"multi_lane-0.csv"),"w");
    d1 = fopen(strcat(outputpath[8],"multi_lane-1.csv"),"w");
    d2 = fopen(strcat(outputpath[9],"multi_lane-2.csv"),"w");
    d3 = fopen(strcat(outputpath[10],"multi_lane-3.csv"),"w");
